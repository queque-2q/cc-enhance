...
class HooksManager {
    ...
    static VERSION_MARKER = 'cc-diff-hooks-v4';
    ...
    getTargetHooksDir(workspaceRoot) {
     ...
     */
    async autoUpdate(workspaceRoot) {
        try {
     ...
     */
    async setupHooks(workspaceRoot) {
    ...
    async copyHooksToTarget(targetDir) {
        const sourceDir = this.getSourceHooksDir();
    ...
    writeVersionMarker(targetDir) {
        fs.writeFileSync(path.join(targetDir, '.version'), HooksManager.VERSION_MARKER, 'utf8');
    }
    /**
     * Merge the cc-diff hook configuration into the project's
     * .claude/settings.json. Preserves existing settings.
     */
    async updateClaudeSettings(workspaceRoot, hooksDir) {
        ...
        const existingPreTool = settings.hooks.PreToolUse.findIndex((h) => h.matcher === preToolUseMatcher);
        ...
        const existingStop = settings.hooks.Stop.findIndex((h) => h.hooks?.some((hh) => typeof hh.command === 'string' && hh.command.includes('session-end.js')));
        if (existingStop >= 0) {
        ...
        if (settings.hooks.SessionEnd) {
            settings.hooks.SessionEnd = settings.hooks.SessionEnd.filter((h) => !h.hooks?.some((hh) => typeof hh.command === 'string' && hh.command.includes('session-end.js')));
