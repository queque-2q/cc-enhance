...
class WebviewProvider {
    ...
    _diffManager;
    _debounceTimer = null;
    constructor(workspaceRoot, diffManager) {
    ...
    async handleMessage(msg) {
        ...
        switch (command) {
            ...
            case 'acceptAll': {
                // Confirmation
                const answer = await vscode.window.showInformationMessage(`Accept all changes in session ${sessionId}?`, { modal: true }, 'Accept All');
            ...
            case 'denyAll': {
                const answer = await vscode.window.showInformationMessage(`Deny (revert) all changes in session ${sessionId}? This will undo all file modifications.`, { modal: true }, 'Deny All');
                if (answer === 'Deny All') {
    ...
    // Diff Editor
    // ======================================================================
    async handleOpenDiff(filePath, sessionId) {
        const reverseContent = this._diffManager.getReverseContent(filePath, this._workspaceRoot, sessionId);
